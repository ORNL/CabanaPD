...
struct HertzianJKRModel : public ContactModel
{
    ...
    double alpha_f; // Maximum separation distance to break contact
    double fc;      // Maximum cohesion (pull-out) force
        ...
        , nu( _nu )
    {
        ...
        alpha_f = Kokkos::pow( 3.0 * Kokkos::pow( fc, 2.0 ) /
                                   ( 16. * Rs * Kokkos::pow( Es, 2.0 ) ),
                               1.0 / 3.0 );
    }
