...
struct TimingOutput<ForceType, Empty>
{
               ...
               const double p_steps_per_sec )
    {
        ...
        log( out, std::fixed, std::setprecision( 6 ), step, "/", num_steps, " ",
             std::scientific, std::setprecision( 2 ), step * dt, " ", W, " ",
             std::fixed, total, " ", force, " ", comm, " ", integrate, " ",
...
template <>
struct TimingOutput<Contact, Empty>
{
    void header( const std::string output_file, const double init )
    {
        std::ofstream out( output_file, std::ofstream::app );
    ...
    void step( const std::string output_file, const int step,
               const int num_steps, const double dt, const double,
               const double total, const double force, const double,
...
template <typename ForceType>
struct TimingOutput<ForceType, Contact>
{
    void header( const std::string output_file, const double init )
    {
        std::ofstream out( output_file, std::ofstream::app );
               ...
               const double p_steps_per_sec )
    {
        ...
        log( out, std::fixed, std::setprecision( 6 ), step, "/", num_steps, " ",
             std::scientific, std::setprecision( 2 ), step * dt, " ", W, " ",
             std::fixed, total, " ", force, " ", contact, " ", neigh, " ", comm,
             " ", integrate, " ", energy, " ", output, " ", std::scientific,
             p_steps_per_sec );
        out.close();
    }
    ...
    void final( const std::string output_file, const int size,
                const int global_particles, const double total,
                const double force, const double contact, const double neigh,
                const double contact_neigh, const double comm,
                const double integrate, const double energy,
                ...
                const double steps_per_sec, const double p_steps_per_sec )
    {
        ...
        log( out, std::fixed, std::setprecision( 2 ),
             ...
             size, " ", global_particles, " | \t", total, " ", force, " ",
             contact, " ", neigh, " ", contact_neigh, " ", comm, " ", integrate,
             " ", energy, " ", output, " ", other, " | PERFORMANCE\n",
             std::fixed, size, " ", global_particles, " | \t", 1.0, " ",
             force / total, " ", contact / total, " ", neigh / total, " ",
             contact_neigh / total, " ", comm / total, " ", integrate / total,
             " ", energy / total, " ", output / total, " ", other / total,
             " | FRACTION\n\n",
             "#Steps/s Particle-steps/s Particle-steps/proc/s\n",
             std::scientific, steps_per_sec, " ", p_steps_per_sec, " ",
             p_steps_per_sec / size );
