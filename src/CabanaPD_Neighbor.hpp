  ...
  public:
        ...
        : _half_neigh( half_neigh )
    {
        ...
        if constexpr ( !is_contact<ModelType>::value )
            _neigh_list = list_type(
                particles.sliceReferencePosition(), particles.frozenOffset(),
                particles.localOffset(), model.cutoff() + tol, 1.0,
    ...
    void update( ParticleType& particles, const double cutoff )
    {
        ...
        _neigh_list.build( exec_space{}, y, n_frozen, n_local, cutoff, 1.0,
                           particles.ghost_mesh_lo, particles.ghost_mesh_hi );
    }
    ...
    template <typename ExecSpace, typename FunctorType, typename ParticleType>
    auto reduce( ExecSpace, const FunctorType functor,
                 const ParticleType& particles, const std::string label ) const
    {
        auto policy = makePolicy<ExecSpace>( particles );
        double total = 0.0;
    ...
    template <typename ExecSpace, typename FunctorType, typename ParticleType>
    auto reduceLinear( ExecSpace, const FunctorType functor,
                       const ParticleType& particles,
                       const std::string label ) const
    {
        double total = 0.0;
        auto policy = makePolicy<ExecSpace>( particles );
        Kokkos::parallel_reduce( label, policy, functor, total );
        return total;
    }
