...
class Neighbor<MemorySpace, Fracture> : public Neighbor<MemorySpace, NoFracture>
{
  ...
  public:
    ...
    // Reallocate mu with global max-neighbor count to match plastic view allocation
    template <typename ParticleType>
    void updateMu( const ParticleType& particles, const int max_neighbors_global )
    {
        Kokkos::realloc( _mu, particles.localOffset(), max_neighbors_global );
        Kokkos::deep_copy( _mu, 1 );
    }
};
