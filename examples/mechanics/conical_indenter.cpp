...
// Simulate a conical indenter on a plate.
void conicalIndenterExample( const std::string filename )
{
    // ====================================================
    ...
    */
    ...
    auto disp_func = KOKKOS_LAMBDA( const int pid, const double t )
    {
        ...
        if ( z_indenter < z_top )
        {
            double r_indenter = ( z_top - z_indenter ) * tan_alpha;
            ...
            if ( r_sq <= r_indenter * r_indenter )
                u( pid, 2 ) = ( z_indenter + std::sqrt( r_sq ) / tan_alpha ) -
                              x( pid, 2 );
        }
