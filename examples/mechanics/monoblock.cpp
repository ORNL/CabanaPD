...
void fiberReinforcedCompositeExample( const std::string filename )
{
    ...
    auto init_functor = KOKKOS_LAMBDA( const int pid )
    {
        ...
        if ( dist > radius - coolant_tubing_width )
        {
            if ( dist < radius - coolant_tubing_width + 0.0015 )
            {
                type( pid ) = 2;
            }
            else if ( dist < radius - coolant_tubing_width + 0.0025 )
            {
                type( pid ) = 1;
            };
        };
