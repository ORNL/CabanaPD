...
macro(CabanaPD_add_tests)
  cmake_parse_arguments(CABANAPD_TEST "MPI" "PREFIX" "NAMES" ${ARGN})
  set(CABANAPD_TEST_MPIEXEC_NUMPROCS 1)
  if(CABANAPD_TEST_MPI)
    list(APPEND CABANAPD_TEST_MPIEXEC_NUMPROCS 2)
    if(MPIEXEC_MAX_NUMPROCS GREATER 2)
      list(APPEND CABANAPD_TEST_MPIEXEC_NUMPROCS ${MPIEXEC_MAX_NUMPROCS})
    endif()
  endif()
  set(CABANAPD_TEST_NUMTHREADS 1 2)

  set(CABANAPD_TEST_MAIN ${PROJECT_SOURCE_DIR}/test_harness/mpi_unit_test_main.cpp)
  ...
  foreach(_device SERIAL PTHREAD OPENMP CUDA HIP)
    if(Kokkos_ENABLE_${_device})
      ...
      foreach(_test ${CABANAPD_TEST_NAMES})
        set(_file ${_dir}/tst_${CABANAPD_TEST_PREFIX}_${_test}_${_uppercase_device}.cpp)
        file(WRITE ${_file}
          "#include <Test${_uppercase_device}_Category.hpp>\n"
          "#include <tst${_test}.hpp>\n"
          )
        set(_target ${CABANAPD_TEST_PREFIX}_${_test}_test_${_uppercase_device})
        add_executable(${_target} ${_file} ${CABANAPD_TEST_MAIN})
        ...
        foreach(_np ${CABANAPD_TEST_MPIEXEC_NUMPROCS})
          if(_device STREQUAL PTHREAD OR _device STREQUAL OPENMP)
            foreach(_thread ${CABANAPD_TEST_NUMTHREADS})
